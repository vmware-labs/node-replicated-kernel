...
# ARG_OPTIONAL_SINGLE([cmd],[c],[Optional qemu arguments.])
# ARG_OPTIONAL_SINGLE([qemu],[q],[Command line for kernel.])
# ARG_OPTIONAL_REPEATED([mods],[m],[Modules to include on startup.],['init', 'rkapps'])
...
{
	local first_option all_short_options='fuascqmrnh'
	first_option="${1:0:1}"
...
print_help()
{
	...
	printf 'Usage: %s [-f|--kfeatures <arg>] [-u|--ufeatures <arg>] [-a|--nodes <arg>] [-s|--cores <arg>] [-c|--cmd <arg>] [-q|--qemu <arg>] [-m|--mods <arg>] [-r|--(no-)release] [-n|--(no-)norun] [-h|--help]\n' "$0"
	...
	printf '\t%s\n' "-c, --cmd: Optional qemu arguments. (no default)"
	printf '\t%s\n' "-q, --qemu: Command line for kernel. (no default)"
	printf '\t%s' "-m, --mods: Modules to include on startup. (default array elements:"
	printf " '%s'" 'init', 'rkapps'
	printf ')\n'
...
USER_BUILD_ARGS="--color always --target=$USER_TARGET"
...
# We need to make sure we build a static vibrio library we can link against
# for rump applications (`rkapps`)
cd ../lib/vibrio
RUST_TARGET_PATH=`pwd`/../../usr xargo build --features rumprt $USER_BUILD_ARGS
...
if [ "${_arg_mods}" != "" ]; then
	...
	do
		...
		if [ "${item}" != "rkapps" ]; then
			...
			# Move all rump apps to the ESP dir:
			cp ${USER_BUILD_DIR}/*.bin $ESP_DIR/
		fi
...
BUILD_ARGS="--color always --target=$BESPIN_TARGET"
