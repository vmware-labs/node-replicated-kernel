...
if [ "${_arg_norun}" != "on" ]; then
    ...
    qemu-system-x86_64 $KVM_ARG -nographic -device isa-debug-exit,iobase=0xf4,iosize=0x04 $QEMU_UEFI_APPEND $QEMU_NET_APPEND $CMDLINE_APPEND $QEMU_MONITOR ${_arg_qemu}
    QEMU_EXIT=$?
    set +ex
