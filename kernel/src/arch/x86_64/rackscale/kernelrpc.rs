...
pub(crate) const KernelRpcRes_SIZE: MsgLen = core::mem::size_of::<KResult<(u64, u64)>>() as MsgLen;
...
pub(crate) fn construct_ret_extra_data(
    ...
    hdr.msg_len =
        KernelRpcRes_SIZE + <u64 as TryInto<MsgLen>>::try_into(additional_data_len).unwrap();
}
