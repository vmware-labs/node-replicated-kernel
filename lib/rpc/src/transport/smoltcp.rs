...
pub struct TCPTransport<'a> {
    ...
    server_handle: Arc<Mutex<SocketHandle>>,
...
impl TCPTransport<'_> {
    pub fn new(
        ...
        let server_handle = Arc::new(Mutex::new(iface.lock().add_socket(tcp_socket)));
        ...
        loop {
            {
                ...
                while socket.can_send() && bytes_sent != 0 {
                    ...
                    if let Ok(bytes_sent) = socket.send_slice(&send_buf[offset..]) {
                        ...
                        panic!("send_slice failed... trying again?");
                    }
                }
            }
    ...
    fn recv(&self, recv_buf: &mut [u8], server_handle: &SocketHandle) -> Result<(), RPCError> {
            ...
            // or no progress is being made (e.g., recv_slice starts returning 0)
            {
                ...
                while socket.can_recv() && bytes_recv != 0 {
                    ...
                    if let Ok(bytes_recv) = socket.recv_slice(&mut recv_buf[offset..]) {
                        ...
                        panic!("recv_slice failed... trying again?");
                    }
                }
            }
    ...
    fn recv_msg(
        ...
        loop {
            {
                let server_handle = self.server_handle.lock();
                let mut iface = self.iface.lock();
                let socket = iface.get_socket::<TcpSocket>(*server_handle);

                let ready_to_recv = if let Some(id) = msg_id {
                    let hdr_slice = socket.peek(1).map_err(|_| RPCError::TransportError)?;
                    hdr_slice.len() == 1 && hdr_slice[0] == id
                } else {
                    socket.can_recv()
                };
                ...
                if ready_to_recv {
                    ...
                    // if we're non-blocking, return now.
                    return Ok(false);
                }
            }
        ...
        // At this point -> we either don't have to match a message id, or we know we have the right
        // message id.
        let server_handle = self.server_handle.lock();
        ...
        let hdr_slice = unsafe { hdr.as_mut_bytes() };
        self.recv(&mut hdr_slice[offset..], &server_handle)?;

        //TODO: should be debug assert?
        if let Some(id) = msg_id {
            assert_eq!(hdr.msg_id, id);
        }
        ...
        if expected_data > max_recv_data {
            ...
            for p in payload.iter_mut() {
                if recv_count + p.len() > expected_data {
                    ...
                    self.recv(&mut p[..(expected_data - recv_count)], &server_handle)?;
                    return Ok(true);
                } else {
                    trace!("recv_msg recv payload buf[{:?}-{:?}]", 0, p.len());
                    recv_count += p.len();
                    self.recv(p, &server_handle)?;
                }
            }
            Ok(true)
        }
...
impl Transport for TCPTransport<'_> {
    ...
    fn send_msg(&self, hdr: &RPCHeader, payload: &[&[u8]]) -> Result<(), RPCError> {
        ...
        let server_handle = self.server_handle.lock();
        self.send(&unsafe { hdr.as_bytes() }[..], false, &server_handle)?;
        for p in payload {
            trace!("send_msg - sending payload");
            self.send(p, false, &server_handle)?;
        }
        Ok(())
    }
    ...
    fn try_send_msg(&self, hdr: &RPCHeader, payload: &[&[u8]]) -> Result<bool, RPCError> {
        ...
        let server_handle = self.server_handle.lock();
        match self.send(&unsafe { hdr.as_bytes() }[..], true, &server_handle)? {
            true => {
                for p in payload {
                    trace!("send_msg - sending payload");
                    self.send(p, false, &server_handle)?;
    ...
    fn recv_msg(
        ...
        self.recv_msg(msg_id, hdr, payload, false)?;
        Ok(())
    }
    ...
    fn try_recv_msg(
        ...
        self.recv_msg(msg_id, hdr, payload, true)
    }
    ...
    fn client_connect(&mut self) -> Result<(), RPCError> {
        let server_handle = self.server_handle.lock();
        {
            ...
            let (socket, cx) = iface.get_socket_and_context::<TcpSocket>(*server_handle);
