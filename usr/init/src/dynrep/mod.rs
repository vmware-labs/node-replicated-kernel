...
impl Default for HashTable {
    fn default() -> Self {
        let allocator = MyAllocator {
            node_id: NODE_ID.fetch_add(1, Ordering::SeqCst),
        };
...
fn run_bench(
    ...
    core_id: usize,
    ttkn: ThreadToken,
    replica: Arc<RwLock<NodeReplicated<HashTable>>>,
) {
    ...
    while iterations <= duration {
        ...
        while start.elapsed().as_secs() < 1 {
            for _i in 0..batch_size {
                ...
                let _ = replica
                    .read(core_id)
                    .execute(OpRd::Get(random_key), ttkn)
                    .unwrap();
                ops += 1;
            }
        }
...
unsafe extern "C" fn bencher_trampoline(args: *mut u8) -> *mut u8 {
    ...
    log::warn!("Before At first barrier");

    // TODO: Add a second replica if needed
    // let machine_id = 0;
    if mid == 2 && kpi::system::mtid_from_gtid(current_gtid) == 0 {
        log::warn!("SPECIAL THREAD 1");
        let replica: Arc<RwLock<NodeReplicated<HashTable>>> =
            Arc::from_raw(args as *const RwLock<NodeReplicated<HashTable>>);
        log::warn!("SPECIAL THREAD 2");

        replica
            .write(current_gtid)
            .add_replica(machine_id)
            .expect("Failed to add replica 1");
        log::warn!("SPECIAL THREAD 3");
    }

    log::warn!("At first barrier");
    ...
    let ttkn = replica.read(current_gtid).register(machine_id).unwrap();

    // Synchronize with all cores
    POOR_MANS_BARRIER.fetch_add(1, Ordering::Release);
    while POOR_MANS_BARRIER.load(Ordering::Acquire) != hwthreads.len() {
        core::hint::spin_loop();
    }
...
pub fn userspace_dynrep_test() {
    ...
    let num_replicas = NonZeroUsize::new(1).unwrap();

    let replicas = Arc::new(RwLock::new(
        NodeReplicated::<HashTable>::new(num_replicas, |_| 0).unwrap(),
    ));
